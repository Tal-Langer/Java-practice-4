...
public class Mines {
	...
	public Mines(int height, int width, int numMines) {// constructor
		...
		if(numMines>=height*width)
			this.numMines=height*width-1;
		else this.numMines = numMines;
		this.width = width;
		this.height = height;
		showAll=false;
		set_places();
		Set_random_mines(this.numMines);
...
/*up=row-- , down = row++ , left = col-- , right = col++ */
	...
	public boolean isDone() {
		for (int i = 0; i <height ; i++)
			for (int j = 0; j < width; j++) {
				if (board[i][j].is_open == 0 && board[i][j].mine_flag == 0 || board[i][j].is_open == 1 && board[i][j].mine_flag == 1  )// not mine && not open
					return false;
